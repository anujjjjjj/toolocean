import { useParams, useNavigate } from "react-router-dom";
import { useEffect, useState } from "react";
import { Button } from "@/components/ui/button";
import { ArrowLeft } from "lucide-react";
import { Header } from "@/components/layout/Header";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { pdfComponentRegistry } from "@/lib/pdfToolRegistry";

// PDF tool metadata
const pdfToolsData = [
    {
        id: "pdf-merge",
        name: "PDF Merge",
        description: "Combine multiple PDF files into one document",
        icon: "Merge",
    },
    {
        id: "pdf-split",
        name: "PDF Split",
        description: "Extract specific pages from a PDF document",
        icon: "Split",
    },
    {
        id: "pdf-compress",
        name: "PDF Compress",
        description: "Reduce PDF file size while maintaining quality",
        icon: "Shrink",
    },
    {
        id: "pdf-to-images",
        name: "PDF to Images",
        description: "Convert PDF pages to PNG or JPG images",
        icon: "Image",
    },
    {
        id: "images-to-pdf",
        name: "Images to PDF",
        description: "Combine multiple images into a single PDF",
        icon: "FileText",
    },
    {
        id: "pdf-rotate",
        name: "PDF Rotate",
        description: "Rotate PDF pages by 90°, 180°, or 270°",
        icon: "RotateCw",
    },
    {
        id: "pdf-watermark",
        name: "PDF Watermark",
        description: "Add text or image watermark to PDF pages",
        icon: "Droplets",
    },
    {
        id: "pdf-reorder",
        name: "PDF Page Reorder",
        description: "Rearrange PDF pages with drag and drop",
        icon: "ArrowUpDown",
    },
];

const PdfToolPage = () => {
    const { toolId } = useParams<{ toolId: string }>();
    const navigate = useNavigate();
    const [searchQuery, setSearchQuery] = useState("");

    const tool = pdfToolsData.find(t => t.id === toolId);

    useEffect(() => {
        if (!tool) {
            navigate("/pdf-tools");
        }
    }, [tool, navigate]);

    if (!tool) {
        return null;
    }

    const renderToolComponent = () => {
        const ToolComponent = pdfComponentRegistry[tool.id];

        if (ToolComponent) {
            return <ToolComponent />;
        }

        return (
            <div className="text-center py-12">
                <h3 className="text-lg font-semibold mb-2">Tool Coming Soon</h3>
                <p className="text-muted-foreground">
                    {tool.name} is being implemented. Check back shortly!
                </p>
            </div>
        );
    };

    return (
        <div className="min-h-screen bg-background">
            <Header onSearch={setSearchQuery} searchQuery={searchQuery} />

            <div className="container mx-auto px-4 py-8">
                {/* Back Button */}
                <Button
                    variant="ghost"
                    onClick={() => navigate("/pdf-tools")}
                    className="mb-6"
                >
                    <ArrowLeft className="h-4 w-4 mr-2" />
                    Back to PDF Tools
                </Button>

                {/* Tool Header */}
                <Card className="mb-6">
                    <CardHeader>
                        <CardTitle className="text-2xl font-heading">
                            {tool.name}
                        </CardTitle>
                        <CardDescription className="text-base">
                            {tool.description}
                        </CardDescription>
                    </CardHeader>
                </Card>

                {/* Tool Component */}
                <Card>
                    <CardContent className="pt-6">
                        {renderToolComponent()}
                    </CardContent>
                </Card>
            </div>
        </div>
    );
};

export default PdfToolPage;
