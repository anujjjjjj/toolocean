...
export function CsvConverterTool() {
    ...
    const csvRows = data.map((row: Record<string, unknown>) =>
      headers
        .map((header) => {
          ...
          return typeof value === "string" && value.includes(actualDelimiter) ? `"${value}"` : String(value);
        })
  ...
  const downloadFile = () => {
    ...
    toast({ title: "File downloaded", description: `converted.${extension} has been downloaded` });
  };
  ...
  return (
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center gap-4">
            <span className="text-sm font-medium">{mode === "csv-to-json" ? "CSV → JSON" : "JSON → CSV"}</span>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle>{mode === "csv-to-json" ? "CSV Input" : "JSON Input"}</CardTitle>
          </CardHeader>
          <CardContent>
            <Textarea
              placeholder={
